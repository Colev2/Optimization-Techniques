...
for i = 1:3
    for j = 1:sampleSize
        ...
        [xmin, ak_list, bk_list, fcount] = golden_section(funcs{i}, a, b, l);
...
title('Πλήθος υπολογισμών f(x) ως προς l (Μέθοδος Χρυσού Τομέα)');
legend('f1', 'f2', 'f3');
grid on;

% Save Figure
saveas(gcf, 'golden_section_part1_l_plot.png');
...
for i = 1:3    % ενα figure για καθε συναρτηση f_i
    ...
    title(t, sprintf('Μέθοδος Χρυσού Τομέα: Σύγκλιση άκρων για f_%d(x)', i), ...
        'FontSize', 16);
    ...
    for li = 1:numel(lValues)
        ...
        title(sprintf('l = %.6f', l), 'FontSize', 13);
        xlabel('Επανάληψη k');
        ylabel('Τιμή άκρου');
        grid on;
    ...
    % Save figure
    saveas(gcf, sprintf('golden_section_ak_bk_f%d.png', i));
end
...
fprintf('\n--- Χρυσός Τομέας : έλεγχος ακρίβειας ---\n');
for i = 1:3
    [xmin, ~, ~, ~] = golden_section(funcs{i}, a, b, l_test);
    fxmin = funcs{i}(xmin);
